...
async function start() {
  ...
  const client = await wa.create({ sessionId: config.sessionId, multiDevice: true, headless: config.headless, qrTimeout: config.qrTimeout, authTimeout: 60, cacheEnabled: true });
